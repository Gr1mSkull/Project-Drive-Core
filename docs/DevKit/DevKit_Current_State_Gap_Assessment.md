...
| Logic identical to Gen1 Rev.A | docs/008 §2.1 | CONFLICT / TBD | Candidate only | docs/007 allows STM32G431 DevKit-only | **Proposed ADR-016** (EDL-001 wins for gates; not Accepted) | Architecture Review WP-008 | P0 |
| Radio identical to Gen1 Rev.A | docs/008 §2.1 | TBD | Candidate only | Physical vs interface equivalence undefined | **Proposed ADR-017** | Architecture Review WP-008 | P0 |
| Same firmware binary as Gen1 | docs/008 §2.4 | CONFLICT | Candidate | MCU alt + no FW projects | **Proposed ADR-018** (Option B model) | Architecture Review WP-008 | P0 |
| Power Rev.DK reduced channels | docs/008; docs/002 §10 | PARTIAL | Consistent candidate counts | Connector wording conflict | Freeze capabilities **Proposed ADR-019** | DevKit electrical architecture WP | P0 |
...
| Absence of HS60 on DevKit | docs/008 | PARTIAL | Consistent statement | Whether any high-current class mandatory | **Proposed ADR-020** | Architecture Review WP-008 | P0 |
...
| 13.8 V / 30 A bench limits | docs/008 | TBD | Candidates only | Not authoritative freeze | TBD-DK-001…003 Open; **Proposed ADR-021** (architecture; numerics Open) | Threshold WP | P0 |
| Exact lamp wattages / loads | docs/008 | CANDIDATE | — | hardware README / yaml partial mismatch | Treat as example loads | Fixture WP | P2 |
| Kill / watchdog numeric limits | docs/008 A6/A7 | TBD | Candidates only | Vague “immediately” | TBD-DK-004/005/007/014/021 Open; **Proposed ADR-022** | Threshold WP | P0 |
...
| Physical fault injection | docs/008 §12; DC-DCC-PWR-089 | PARTIAL / TBD | Proposed power reqs | Safe methods undefined | **Proposed ADR-023** | Fixture WP after ADR acceptance | P0 |
| Representative class coverage vs Gen1 | DC-DCC-PWR-108 Proposed | PARTIAL | Count differences allowed | Class set not frozen | **Proposed ADR-019/020** | Architecture Review WP-008 | P0 |
...
| `firmware/dcc/logic` / `radio` | firmware/ | NOT PRESENT | shared headers only | — | Bring-up after **Proposed ADR-018** Accepted | Firmware WP | P1 |
...
| 1.2 | 2026-07-20 | WP-008 — map P0 gaps to Proposed ADR-016…023; no TBD closures; conflict G431 vs EDL-001 still reported |
